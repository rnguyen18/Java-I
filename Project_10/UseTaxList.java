         ...
         switch (vehicleType.toUpperCase().charAt(0))
         {
            ...
            case 'S':
               vehicleList[count] = new SemiTractorTrailer(
                  owner, vehicleName, price, altFuel,
                     Double.parseDouble(scanLine.next().trim()), // Weight in tons
                     Integer.parseInt(scanLine.next().trim())); // Axles
               break;
           
            case 'M':
               vehicleList[count] = new Motorcycle(
                  owner, vehicleName, price, altFuel,
                     Double.parseDouble(scanLine.next().trim())); // Large Bike Threshhold
               break;
               
            default:
               this.addExcludedRecord(thisLine);
               count--;
               break;
         }
         count++; 
